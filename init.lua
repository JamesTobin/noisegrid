...
minetest.register_on_generated(function(minp, maxp, seed)
    ...
    local tree_locations = {}  -- because they can't be grown while the terrain is being generated
    -- noisegrid_heightmap = {}
                ...
                if chunk then
                    ...
                    elseif y == ysurf and y > YSAND then
                        ...
                        elseif stable[si] >= 2 and nodid ~= c_roadblack and
                            ...
                            -- basic_houses mod
                            if minetest.get_modpath("basic_houses") and false then -- maybe some time
                                house()  -- of course this is the correct function
                            
                            -- trees
                            elseif math.random() < APPCHA * n_tree and
                                    n_abspath > 0.015 and
                                    n_abspath2 > 0.015 and
                                    n_absroad > 0.02 and
                                    (n_absalt > 0.04 or y > YFLAT) then
                                table.insert(tree_locations, {x=x, y=y+1, z=z})
                                -- noisegrid_tree(x, y + 1, z, area, data)
                            else
                                data[vi] = c_grass
                                -- flowers
                                ...
                                elseif math.random() < GRACHA * n_grass and
                                        n_absroad > 0.015 and
                                        n_absalt > 0.03 then
                                    if math.random(20) <= 1 and minetest.get_modpath("farming") then
                                        noisegrid_crop(data, via)
                                    else
    ...
    -- grow trees here
    for i, pos in pairs(tree_locations) do
        noisegrid_tree(pos)
    end
